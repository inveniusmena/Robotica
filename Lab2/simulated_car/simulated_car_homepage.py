...
def car_dynamics(state):
    _, _, car_angle, v_u, phi, u_2 = state
    ...
    dx = v_u * ( math.cos(car_angle) -b/L *math.tan(car_angle)*math.sin(car_angle))
    dy = v_u *( math.sin(car_angle) +b/L *math.tan(car_angle)*math.cos(car_angle))
...
def draw_text_with_glow(text, position, is_selected, glow_color=(255, 200, 0), text_color=(255, 255, 255), selected_text_color=(0, 0, 0)):
    """Draw text with glow and rectangle only when selected."""
    x, y = position
    option_width, option_height = font.size(text)

    temp_surface = pygame.Surface((option_width + 20, option_height + 15), pygame.SRCALPHA)
    ...
    if is_selected:
        ...
        pygame.draw.rect(temp_surface, (255, 255, 255, 255), (0, 0, option_width + 20, option_height + 15), 3)
...
def draw_home_screen(selected):
    ...
    welcome_text = font.render("Welcome to the Car Simulator", True, (255, 255, 255))
    welcome_text = pygame.transform.scale(welcome_text, (welcome_text.get_width() * 1.5, welcome_text.get_height() * 1.5))
    screen.blit(welcome_text, (WIDTH // 2 - welcome_text.get_width() // 2, HEIGHT // 2 - welcome_text.get_height() // 2 - 150))

    title_text = font.render("Choose a Mode", True, (255, 255, 255))
    title_text = pygame.transform.scale(title_text, (title_text.get_width() * 1.22, title_text.get_height() * 1.2))
    screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, HEIGHT // 3 + 170))

    options = ["Joystick Mode", "Dynamic Mode", "Kinematics Mode"]
    help_option = "Help"
    
    for i, option in enumerate(options):
        pos = (WIDTH // 2 - font.size(option)[0] // 2, HEIGHT // 3 + 240 + i * 60)
        draw_text_with_glow(option, pos, selected == i)
    # Draw the Help button in the bottom right corner
    help_pos = (WIDTH - font.size(help_option)[0] - 20, HEIGHT - font.size(help_option)[1] - 20)
...
while running:
    if in_home_screen:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    selected_option = (selected_option - 1) % 4  # Update to 4 options
                    ...
                    if selected_option == 3:  # Help option
                        ...
                        mode = ["joystick", "dynamic", "kinematics"][selected_option]
        ...
        if mode == "joystick":
            ...
            # Call the function in the game loop
            car_x, car_y, car_angle, car_speed = kinematics_mode(keys, car_x, car_y, car_angle, car_speed)
        ...
        if sensor_active:    
            ...
            for angle_offset in range(-SENSOR_ANGLE // 2, SENSOR_ANGLE // 2 + 1, 4):  # Steps within the cone
                sensor_angle = - car_angle - math.radians(- angle_offset)  # Adjust for rotation
        ...
        rotated_car = pygame.transform.rotate(car, -math.degrees(car_angle))  # Negative angle to match screen coordinates
